...
# In-memory store for actions to allow dynamic updates during demo/session
CORRECTIVE_ACTIONS = [
    {
        "id": 1,
        "prediction_id": 1,
...
async def get_corrective_actions(
    ...
    """Get recommended corrective actions sorted with top priority on top."""
    actions = list(CORRECTIVE_ACTIONS)
    ...
    # Always sort with top priority on top (urgent -> high -> medium -> low), then impact descending
    actions.sort(
        key=lambda a: (
            PRIORITY_WEIGHT.get(a.get("priority", "low"), 99),
            -a.get("estimated_impact_tonnes", 0),
        )
    )
...
async def mark_action_implemented(action_id: int):
    """Mark a corrective action as implemented and record into action history."""
    ...
    action["is_implemented"] = True

    # Record into history if not already recorded
    existing = next((h for h in ACTION_HISTORY if h.get("action_id") == action_id), None)
    if not existing:
        history_entry = {
            "id": len(ACTION_HISTORY) + 101,
            "action_id": action_id,
            "mine_name": action["mine_name"],
            "action": action["title"],
            "date_implemented": date.today().isoformat(),
            "predicted_impact_tonnes": action["estimated_impact_tonnes"],
            "actual_impact_tonnes": action["estimated_impact_tonnes"],
            "outcome": "success",
        }
        ACTION_HISTORY.insert(0, history_entry)

    return {
        "success": True,
        "message": f"Action '{action['title']}' marked as implemented successfully.",
        "action": action,
    }
